...
const LabelTableBody: FC<Props> = (props) => {
    ...
    const jsxForClassNameAndColorSection = (level: number,
        ...
        i18nName: string,
        hasChildren: boolean,
        labelsAreAllowed: boolean,
        effectiveColor: ColorDef): JSX.Element => {
    ...
    const jsxForTableRows = (): JSX.Element => {
        ...
        const processItem = (item: MLStateTableDataItem, level: number, isExpanded: boolean, hasChildren: boolean, labelsAreAllowed: boolean) => {
            ...
            if (!onlyShowPresent || FORCE_ALL || item.hasData) {
                ...
                let effectiveColor : ColorDef;

                if (item.name.endsWith("unclassified")) {
                    let startOfUnclassified = item.name.indexOf("unclassified");
                    const baseClassItemName = item.name.substring (0, startOfUnclassified);
                    const baseItem: MLStateTableDataItem | undefined = props.itemMap.get(baseClassItemName);
                    effectiveColor = baseItem!.color;
                }
                else {
                    effectiveColor = item.color;
                }
                    ...
                    tableRows.push(
                        <tr key={'table-row-' + item.name}>
                            <td className="mltc-name-td-v2">
                                {jsxForClassNameAndColorSection(level, isExpanded, item, i18nName, hasChildren, labelsAreAllowed, effectiveColor)}
                            </td>
