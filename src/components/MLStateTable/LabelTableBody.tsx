...
const removeItemFromSelectedItems = (
    props: OwnProps,
    item: MLStateTableDataItem,
    selectedItems: Map<MachineLearningLabel, MLStateTableDataItem>
): void => {
...
const globalCheckboxState = createState(false);
...
const LabelTableBody: FC<Props> = (props) => {
    ...
    //         let box: HTMLInputElement = event.currentTarget;
    ...
    // const itemSelectChangeHandler = <T extends HTMLInputElement>(props: Props) => {
    //     return (event: React.SyntheticEvent<T>) => {
    //             if (props === undefined) {
    //                 return;
    //             }
    //         props.onCheckboxStateChange(
    //             isChecked,
    //
    //     };
    // }
    ...
    // const handleFieldCheckboxClicked = (props: OwnProps) => (): void => {
    //     if (props === undefined) {
    //         return;
    //     }
    //     props.onRemoveSelectedLabelItem(
    //         {
    //             ...props.onRemoveSelectedLabelItem(),
    //
    //             rule: {
        ...
        return <>
            <label>
                <input
                    type="checkbox"
                    checked={isChecked}
                    onChange={props.onCheckboxStateChange(!isChecked, item.name)}
                />
            </label>
    ...
    const jsxForTableRows = (): JSX.Element => {
        ...
        const [anyLabelSelected, labelSectionAttributes, predSectionAttributes] = LabelTable.getSectionAttributes(props);
                    ...
                    tableRows.push(
                        <tr key={'table-row-' + item.name}>
                            <td className="mltc-name-td-v2" style={{whiteSpace: "nowrap"}}>
                                {jsxForNameColumn(level, isExpanded, item, i18nName, hasChildren, isChecked)}
                            </td>
