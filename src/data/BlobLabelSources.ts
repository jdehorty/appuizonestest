...
import { predictionJson } from "./PredictionSource.json"; 
import { labelSource } from "./LabelSource.csv";
...
export class BlobBasedMachineLearningLabelInterface extends MachineLearningLabelInterface {
    ...
    private async _download_user_labels(): Promise<Map<Id64String, MachineLearningLabel>> {
        ...
        const containerName = "abce-labels";

        // const blobName = `${this._config.projectGuid}_${this._config.imodelGuid}_${this._config.revisionId}_labels-jkd.csv`;
        const blobName = "PIPE - EQP Structure_instance-labels_jkd.csv";

        const instanceMap = new Map<Id64String, MachineLearningLabel>();

        try {
            //const blobData = await downloadBlobAsString(this._config.accountName, this._config.sasString, containerName, blobName);
            const blobData = labelSource;
            ...
            try {
                ...
                // const blobData = labelSource.split("\n");
                // if (blobData !== undefined) {
                //     const a = blobData.split("\n");
                //     if (a.length >= 1) {
                //         a.shift();
