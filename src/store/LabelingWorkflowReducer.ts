...
export function LabelingWorkflowManagerReducer(
    ...
    switch (action.type) {
        ...
        case LabelingWorkflowManagerActionType.CheckboxStateWasChanged: {
            const newCommonLabelStateMap = new Map<MLLabelId, CommonLabelState>();
            for (const [MLLabelId, checkboxState] of prevState.commonLabelStateMap) {
                if (MLLabelId === action.MLLabelId) {
                    const newCheckboxState: CommonLabelState = {
                        ...checkboxState,
                        isChecked: action.newCheckboxState!,
                    };
                    newCommonLabelStateMap.set(MLLabelId, newCheckboxState);
                } else {
                    newCommonLabelStateMap.set(MLLabelId, checkboxState);
                }
            }
            return {
                ...prevState,
        ...
        case LabelingWorkflowManagerActionType.ElementLabelsWereChanged: {
            ...
            for (const id of action.elementSet!) {
                ...
                elementState.trueLabel = action.MLLabelId!;
        ...
        case LabelingWorkflowManagerActionType.PredLabelVisibilityWasChanged:
            ...
            const newPredLabelStateMap = new Map<MLLabelId, PredLabelState>(prevState.predLabelStateMap);
            if (action.MLLabelId === undefined) {
        ...
        case LabelingWorkflowManagerActionType.TrueLabelVisibilityWasChanged:
            ...
            const newTrueLabelStateMap = new Map<MLLabelId, TrueLabelState>(prevState.trueLabelStateMap);
            if (action.MLLabelId === undefined) {
                // Change state of all labels
